...
def _wait_for_connection_lost_popup(state: dict, proc: subprocess.Popen, disconnect_triggered: dict) -> bool:
    if not disconnect_triggered["done"]:
        if (
            state.get("current_path") == "/"
            and "starting" in state.get("title", "").lower()
...
def _cdp_capture_page_state(sock: socket.socket, command_id: int) -> dict:
    expression = (
        ...
        "hasConnectionLostOverlay: !!document.getElementById('connection-lost-overlay'),"
...
def test_skip_handoff_browser_detects_login_when_auth_already_enabled(qemu_bootstrap_instance):
    ...
    assert browser_state.get("current_path") == "/login", (
        "Bootstrap handoff did not reach the login page after auth-enabled startup.\n"
        f"Browser URL: {browser_state.get('current_url')}\n"
        f"Browser title: {browser_state.get('title')}\n"
        f"Browser error: {browser_state.get('error')}\n"
        f"DOM snippet:\n{browser_state.get('page_source', '')[:1200]}\n"
    ...
    assert "login" in browser_state.get("title", "").lower(), (
        "Browser did not render the login page after bootstrap handoff.\n"
        f"Browser URL: {browser_state.get('current_url')}\n"
        f"Browser title: {browser_state.get('title')}\n"
        f"Browser error: {browser_state.get('error')}\n"
        f"DOM snippet:\n{browser_state.get('page_source', '')[:1200]}\n"
...
def test_bootstrap_browser_shows_connection_lost_popup_after_disconnect(qemu_bootstrap_instance):
    ...
    browser_state = _capture_browser_state_in_headless_chrome(
        f"{base_url}/",
        wait_condition=lambda state: _wait_for_connection_lost_popup(state, proc, disconnect_triggered),
        wait_s=10.0,
    )
    assert disconnect_triggered["done"], (
        "Browser never reached a stable bootstrap document before the simulated disconnect.\n"
        f"Browser URL: {browser_state.get('current_url')}\n"
        f"Browser title: {browser_state.get('title')}\n"
        f"Browser error: {browser_state.get('error')}\n"
    ...
    assert browser_state.get("has_connection_lost_overlay") is True, (
        "Bootstrap page no longer contains the connection-lost overlay markup.\n"
        f"Browser URL: {browser_state.get('current_url')}\n"
        f"Browser title: {browser_state.get('title')}\n"
        f"Browser error: {browser_state.get('error')}\n"
        f"Body text:\n{browser_state.get('body_text', '')[:1200]}\n"
    ...
    assert browser_state.get("connection_lost_visible") is True, (
        "Bootstrap page did not show the connection-lost popup after the device disappeared.\n"
        f"Browser URL: {browser_state.get('current_url')}\n"
        f"Browser title: {browser_state.get('title')}\n"
        f"Browser error: {browser_state.get('error')}\n"
        f"Body text:\n{browser_state.get('body_text', '')[:1200]}\n"
        f"DOM snippet:\n{browser_state.get('page_source', '')[:1200]}\n"
        f"Log tail before disconnect:\n{_tail_log(log_path)}"
    )
    assert "Connection lost!" in browser_state.get("body_text", ""), (
        "Bootstrap connection-lost overlay did not render the expected text.\n"
        f"Browser URL: {browser_state.get('current_url')}\n"
        f"Browser title: {browser_state.get('title')}\n"
        f"Browser error: {browser_state.get('error')}\n"
        f"Body text:\n{browser_state.get('body_text', '')[:1200]}\n"
